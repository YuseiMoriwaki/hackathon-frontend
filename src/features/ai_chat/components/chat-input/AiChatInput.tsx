...
const AiChatInput = forwardRef<AiChatInputRef, AiChatInputProps>(function AiChatInput(
  { onSend, onStop, isLoading = false, placeholder = 'Ask anything', canStop = true },
  ref
) {
  ...
  const handleSend = useCallback(
    async (
      ...
      const uploadedFiles = uploadItems.filter(item => item.uploaded).map(item => item.uploaded!);
  ...
  const canSend =
    !isLoading && !hasUploadingFiles && (content.trim().length > 0 || uploadItems.length > 0);
          ...
          <div className="relative">
            ...
            <div className="absolute left-0 bottom-0 z-100">
              <FileUploadMenu
                ...
                maxFiles={4}
                disabled={isLoading || hasUploadingFiles || uploadItems.length >= 4}
              />
            </div>
