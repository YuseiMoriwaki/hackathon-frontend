  ...
  return (
    <Link href={`/items/${item.id}`}>
      <div className="glass-card rounded-2xl p-4 hover:glass-card-hover transition-all duration-300 cursor-pointer">
        ...
        <h3 className="font-semibold text-white mb-2 line-clamp-2 text-sm">{item.title}</h3>

        <p className="text-xl font-bold text-blue-300 mb-2">¥{item.price.toLocaleString()}</p>

        <p className="text-xs text-white/50 mb-3">{item.sellerName}</p>
